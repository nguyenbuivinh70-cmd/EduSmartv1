...
function statusPresentation(status: FirebaseBaseCatalogStatus | null) {
  if (!status?.available) {
    return {
      label: 'Chưa sẵn sàng đồng bộ',
      description: 'Hãy đăng nhập lại bằng tài khoản quản trị để thực hiện đồng bộ.',
  ...
  if (status.current) {
    return {
      ...
      description: 'Ba danh mục nền đã đồng bộ và trùng khớp với dữ liệu nguồn.',
  ...
  return {
    label: 'Chưa đồng bộ',
    description: 'Kho dữ liệu hiện tại chưa có đủ Năm học, Lớp học và Môn học.',
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700">
            <Database className="h-3.5 w-3.5" /> Đồng bộ dữ liệu nền
          </div>
          <h2 className="mt-3 text-xl font-bold text-slate-900">Đồng bộ danh mục nền</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
            Hệ thống giữ nguyên mã dữ liệu, cập nhật các bản ghi hiện có và loại bỏ dữ liệu không còn trong nguồn. Dữ liệu dự phòng vẫn được giữ trong giai đoạn chuyển đổi.
          </p>
        </div>
          ...
          return (
            <div key={row.label} className={`rounded-2xl border p-4 ${matches ? 'border-emerald-100 bg-emerald-50/60' : 'border-slate-200 bg-slate-50'}`}>
              ...
              <div className="mt-2 flex items-end justify-between gap-3">
                <div><span className="text-2xl font-extrabold text-slate-900">{row.target}</span><span className="ml-1 text-xs text-slate-500">Hệ thống</span></div>
