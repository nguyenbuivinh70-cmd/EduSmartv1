...
function presentation(status: FirebaseAccountMigrationStatus | null) {
  if (!status?.available) {
    return { label: 'Chưa kiểm tra dữ liệu tài khoản', className: 'bg-slate-100 text-slate-600' };
  }
  if (status.current) {
    ...
    <section className="rounded-[28px] bg-white p-8 shadow-sm ring-1 ring-indigo-100">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700">
            <ShieldCheck className="h-3.5 w-3.5" /> Chuẩn hóa tài khoản
          </div>
          <h2 className="mt-4 text-xl font-bold text-slate-900">Chuẩn hóa tài khoản đăng nhập</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Người dùng tiếp tục sử dụng tên đăng nhập hiện có. Hệ thống tự đồng bộ hồ sơ đăng nhập và giữ dữ liệu dự phòng trong quá trình chuyển đổi.
          </p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <div className="rounded-2xl bg-emerald-50 p-4">
          <div className="text-xs font-bold uppercase tracking-wide text-emerald-700">Đã chuẩn hóa</div>
      ...
      {resetAccounts.length > 0 && (
        <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-semibold">Mật khẩu khởi tạo cần tối thiểu 6 ký tự.</p>
              <p>
                {resetAccounts.map((item) => item.username).join(', ')} sẽ được đặt mật khẩu khởi tạo là <strong>123456</strong>.
                Dữ liệu tài khoản được cập nhật tự động sau khi hoàn tất.
              </p>
            </div>
          </div>
        </div>
      ...
      {status && status.mismatchCount > 0 && (
        <div className="mt-5 rounded-2xl bg-rose-50 px-4 py-3 text-sm text-rose-700">
          Có {status.mismatchCount} hồ sơ chưa khớp vai trò, lớp hoặc trạng thái nguồn. Nhấn chuyển đổi để cập nhật lại.
        </div>
      )}
